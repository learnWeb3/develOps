...
quizRouter.get("/quiz/new", filter(filterOptions), async (req, res, next) => {
  try {
...
quizRouter.post("/quiz", filter(filterOptions), async (req, res, next) => {
  try {
    const { title, questionContent, answer, isValid, article } = req.body;
    await Quiz.register({
      title,
      questionContent,
      answer,
...
quizRouter.post("/quiz/:id", filter(filterOptions), async (req, res, next) => {
  try {
    const { id } = req.params;
    req.session.flash = {
